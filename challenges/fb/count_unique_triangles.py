...
def countDistinctTriangles(arr):
  ...
  unique = set()
  # elements of input array are length 3 always
  # so sorting them can be considered constant time.
  # then, a function of the input size is the loop we have to make
  
  for (index, tri) in enumerate(arr):
    key = hashed(tri)
    unique.add(key)
