...
def count_of_nodes(root, queries, s):
  ...
  N = len(s)
  c_at_n = [None] + [initCounterForChars(s) for _ in range(N)]  # O(N^2) where N is length of S
  S = ' ' + s  # to make it 1-indexed
  
  # Like doing tabulation but traversing a tree (DFS)
  def traverse(node, previous_nodes, frequency_at_node, S):
    # Node is never null. previous_nodes is a list of the ancestors of node (Path)
    i = node.val  # index to S
    c = S[node.val] # char to count for
    frequency_at_node[i][c] += 1  # node i has at least 1 occurrence of c on its subtree
    ...
    for c in node.children:
      traverse(c, previous_nodes + [node], frequency_at_node, S)  # pass along the counter dictionary
...
# TIME: O(N^2) for building the DStruct. Then O(V+E)=O(N+(N-1))=O(N) for DFS. Then O(k) for the number of queries.
# the tradeoff is not very good, just how long could S be? Is the DS worth it?
